...
def getBestMineToLoot(userAddress: Address, team: Team, *args: Any, **kwargs: Any) -> Game:
    """
    ...
    strategyName = User(userAddress).getTeamConfig(team['team_id']).get('lootStrategyName')
    if not strategyName:
        raise StrategyNotSet(f"Could not find the lootStrategyName setting for team {team['team_id']}")
...
def getBestReinforcement(userAddress: Address, mine: Game, maxPrice: Tus, *args: Any, **kwargs: Any) -> CrabForLending:
    """
    ...
    if not teamConfig:
        raise StrategyException(f"Current user {userAddress} has no teams involved in mine {mine['game_id']}") # type: ignore

    # Fetch the strategy name from the user settings
    strategyName = teamConfig.get('reinforceStrategyName')
    if not strategyName:
        raise StrategyNotSet(f"Could not find the reinforceStrategyName setting for team {teamConfig['id']}")
...
def makeLootStrategy(strategyName: str, team: Team, *params: Any, **paramsKw: Any) -> LootStrategy:
    """
...
def makeReinforceStrategy(strategyName: str, mine: Game, maxPrice: Tus, *params: Any, **paramsKw: Any) -> ReinforceStrategy:
    """
    ...
    if not strategyClass:
        raise StrategyNotFound(f"Cound not find a reinforceStrategy named {strategyName}")
    ...
    return strategyClass(crabadaWeb2Client).setParams(mine, maxPrice, *params, **paramsKw)
